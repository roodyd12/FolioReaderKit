...
open class FolioReaderPage: UICollectionViewCell, UIGestureRecognizerDelegate {
    ...
    @objc open func handleTapGesture(_ recognizer: UITapGestureRecognizer) {
        ...
        if let _navigationController = self.folioReader.readerCenter?.navigationController,
            (_navigationController.isNavigationBarHidden == true) {
            
            webView?.js("getSelectedText()", completion: { [weak self] selected in
                guard let weakSelf = self, selected == nil || selected?.isEmpty == true else { return }
     ...
     */
    func getAnchorOffset(_ anchor: String, completion:@escaping ((_ offset:CGFloat) -> Void)) {
        ...
        webView.js(script, completion: { result in
            guard let strOffset = result else {
                completion(0)
                return
            }
...
//            }
    ...
    @objc func refreshPageMode() {
        ...
        if (self.folioReader.nightMode == true) {
            ...
            //TODO: Code changed, test it
            let script = "document.documentElement.offsetHeight"
            let frameHeight = webView.frame.height
            let pageCount = CGFloat(webView.pageCount)
            webView.evaluateJavaScript(script, completionHandler: { [weak self, weak webView] (result, error) in
                guard let webView = webView, let heightString = result as? String, let height = Double(heightString) else { return }
                let lastPageHeight = (frameHeight * pageCount) - CGFloat(height)
    ...
    fileprivate func setupClassBasedOnClickListeners() {
        readerConfig.classBasedOnClickListeners.forEach({ listener in
    ...
    open func performJavaScript(_ javaScriptCode: String) -> String? {
        // TODO: Fix this
        return nil // webView?.js(javaScriptCode)
    }
}
...
extension FolioReaderPage: WKNavigationDelegate {
    ...
    public func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        if self.readerConfig.enableTTS && !self.book.hasAudio {
            let script = "wrappingSentencesWithinPTags()"
            webView.js(script, completion: { [weak self] result in
                guard let audioPlayer = self?.folioReader.readerAudioPlayer, audioPlayer.isPlaying() else { return }
        ...
        if scheme == "highlight" || scheme == "highlight-with-note" {
            ...
            for listener in self.readerConfig.classBasedOnClickListeners {
                ...
                if scheme == listener.schemeName,
                    let range = url.absoluteString.range(of: "/clientX=") {
                    let baseURL = String(url.absoluteString[..<range.lowerBound])
                    let positionString = String(url.absoluteString[range.lowerBound...])
...
// MARK: - WKUIDelegate
extension FolioReaderPage: WKUIDelegate {
